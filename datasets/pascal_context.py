...
class PASCALContext(torch.utils.data.Dataset):
    ...
    def __init__(
        ...
        for x in file_names:
            ...
            _human_part = os.path.join(part_gt_dir, x + ".mat")
            assert (os.path.isfile(_human_part)
                    ), "Didnt found {})".format(_human_part)


            self.parts.append(_human_part)

            # _sal = os.path.join(self.root, sal_gt_dir, x + ".png")
            _sal = os.path.join(sal_gt_dir, x + ".png")
    ...
    def _load_img(self, index):
        _img = np.array(
            Image.open(self.images[index]).convert("RGB"), dtype=np.float32
        )
        return _img
    ...
    def _load_sal(self, index):
        _sal = (
            np.array(Image.open(self.sals[index]), dtype=np.float32) / 255.0
        )  # [0,255] => [0,1]
