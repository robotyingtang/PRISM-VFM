...
def get_transformations(dataname, size, train=True):
    if dataname in ["pascalcontext", "nyud"]:
        if train:
...
class FixedResize(object):
    def __init__(self, size):
        if isinstance(size, int):
            ...
            raise ValueError("Resize size must be an int or a tuple!")

    def resize(self, key, arr):
        arr = np.squeeze(arr)
        interp = cv2.INTER_LINEAR if key == "image" else cv2.INTER_NEAREST
        resized = cv2.resize(arr, (self.size[1], self.size[0]), interpolation=interp)
        if resized.ndim == 2:
