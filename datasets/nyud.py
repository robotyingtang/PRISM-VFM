...
class NYUD(torch.utils.data.Dataset):
    ...
    def __init__(
        ...
        for x in file_names:
            ...
            _edge = os.path.join(edge_gt_dir, x + ".png")
            assert os.path.isfile(_edge)
            self.edges.append(_edge)

            _semseg = os.path.join(semseg_gt_dir, x + ".png")
            assert os.path.isfile(_semseg)
            self.semsegs.append(_semseg)

            _normal = os.path.join(normal_gt_dir, x + ".png")
            assert os.path.isfile(_normal)
            self.normals.append(_normal)

            _depth = os.path.join(depth_gt_dir, x + ".npy")
    ...
    def _load_edge(self, index):
        _edge = (
            np.array(Image.open(self.edges[index]), dtype=np.float32)
            / 255.0
        )
        return _edge
    ...
    def _load_semseg(self, index):
        _semseg = (
            np.array(Image.open(self.semsegs[index]), dtype=np.float32) - 1
        )
        _semseg[_semseg == -1] = 255
        return _semseg

    def _load_normals(self, index):
        _tmp = np.array(Image.open(self.normals[index]), dtype=np.float32)
